...
from dataset.robot_pseudo_dataset_1008 import robot_pseudo_DataSet
...
def get_arguments():
    ...
    parser.add_argument("--input-size", type=str, default='1280,960',
                        help="Comma-separated string with height and width of source images.")
    parser.add_argument("--crop-size", type=str, default='640, 360',
                        help="Comma-separated string with height and width of source images.")
    parser.add_argument("--data-dir-target", type=str, default='./data/Oxford_Robot_ICCV19',
                        help="Path to the directory containing the target dataset.")
    parser.add_argument("--data-list-target", type=str, default='./dataset/robot_list/train.txt',
                        help="Path to the file listing the images in the target dataset.")
    parser.add_argument("--input-size-target", type=str, default='1280,960',
    ...
    parser.add_argument("--label-path", type=str, default='./transition/GTA/pseudo/2',
                        help="Path to the directory of log.")
    parser.add_argument("--label_choice", type=str, default='mean',
                        help="whether to use mean or min prov labels")
...
if __name__ == '__main__':
    ...
    trainloader = data.DataLoader(
        robot_pseudo_DataSet(root=args.data_dir, label_path=args.label_path, list_path=args.data_list,
                                  max_iters=args.num_steps * args.iter_size * args.batch_size,
                                  resize_size=args.input_size,
                                  crop_size=args.crop_size,
                                  scale=True, mirror=True, mean=IMG_MEAN,
                                  set='train', autoaug=args.autoaug, label_choice=args.label_choice),
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            images, labels, labels_prob, _, _ = batch
            images = images.cuda()
            labels = labels.long().cuda()
            labels_prob = labels_prob.long().cuda()
